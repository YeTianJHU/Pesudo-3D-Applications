...
class kineticsDataset(Dataset):
	def __init__(self, data_folder, split_file, label_file, transform, num_labels=400, num_frame=16, channel=3, size=160):

		self.data_folder = data_folder
		self.transform = transform
		self.video_content = self.read_csv_file(split_file)
		self.num_frame = num_frame
		self.channel = channel
		self.size = size
		self.label_dict = self.trans_label(label_file)
	...
	def __getitem__(self, index):
		...
		# print video_name
		video_path = self.get_vid_dir(self, index)
	...
	def __len__(self):
		return len(self.video_content)

	def read_csv_file(self, dir):
		content = []
		with open(dir) as f:
			f_csv = csv.reader(f)
			headers = next(f_csv)
	...
	def get_vid_dir(self, index):
		dir = join(self.data_folder, self.video_content[index][0])
		dir = join(dir, self.video_content[index][1]+"*")
		return dir
	...
	def get_video_tensor(self, dir, num_frame, channel, size):
		...
		flow = torch.FloatTensor(channel,num_frame,size,size)
		seed = np.random.random_integers(0,len(video_tensor)-num_frame) #random sampling
		for i in range(num_frame):
			img = Image.open(video_tensor[i+seed])
			img = img.convert('RGB')
			img = self.transform(img)
			flow[:,i,:,:] = img
		return flow
